...
impl LinkedListAllocator {
    ...
    unsafe fn add_free_region(&mut self, addr: usize, size: usize) {
        // ensure that the freed region is capabale of holding ListNode
    ...
    fn find_region(&mut self, size: usize, align: usize) -> Option<(&'static mut ListNode, usize)> {
        ...
        while let Some(ref mut region) = current.next {
            ...
            // region is not suitable for allocation -> move to the next node
    ...
    fn alloc_from_region(region: &ListNode, size: usize, align: usize) -> Result<usize, ()> {
        ...
        if alloc_end > region.end_addr() {
            // the region is too small for the allocation
            return Err(());
        }

        let excess_size = region.end_addr() - alloc_end;
        if excess_size > 0 && excess_size < mem::size_of::<ListNode>() {
            // the rest of region too small to hold a ListNode (required because the allocation
